...
@Component({
  selector: 'app-edit-sales-quotation',
  templateUrl: './edit-sales-quotation.component.html',
  styleUrls: ['./edit-sales-quotation.component.css']
})
export class EditSalesQuotationComponent implements OnInit {
  ...
  public checkDisc(evnt){
    if (evnt.target.value >= this.total) {
      this.f.discount.setValue(0);
    }
  }
  initFormProduct() {
  ...
  delete() {
    if (this.deletedProIds.length > 0) {
      this.deletedProIds.forEach(id => {
        this.quotationService.deleteSalesQuotationItemById(id).subscribe(res => {
          if (res.code === 200) {
  ...
  public getQuotation(id) {
    this.quotationService.getSalesQuotationById(id).subscribe(result => {
      if (result.code === 200) {
  ...
  public bindingData(data) {
    ...
    this.f.quoteno.setValue(data.quoteno);
    this.f.quoteDate.setValue(this.constants.convert(data.quoteDate));
    ...
    this.f.seNo.setValue(data.seNo);
