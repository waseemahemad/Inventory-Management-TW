...
@Component({
  selector: 'app-edit-sales-delivery-note',
  templateUrl: './edit-sales-delivery-note.component.html',
  styleUrls: ['./edit-sales-delivery-note.component.css']
})
export class EditSalesDeliveryNoteComponent implements OnInit {
  ...
  delete() {
    if (this.deletedProIds.length > 0) {
      this.deletedProIds.forEach(id => {
        this.deliveryService.deleteDeliveryNoteItemById(id).subscribe(res => {
          if (res.code === 200) {
  ...
  public getDeliveryById(id) {
    this.deliveryService.getDeliveryNoteById(id).subscribe(result => {
      if (result.code === 200) {
  ...
  public bindingData(data) {
    ...
    this.itemDto = data.deliveryNoteItems;
